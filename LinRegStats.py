...
# The use of statistical tools and analysis to generate algorithms
# to predict outcomes. One basic example of this is
# linear regression - which wil be covered in this lesson.

# -----import numpy---------
import numpy as np
# for particular numpy version:
...
# while using the values of slope and intercept,
# what a future or predicted value would be based on
...
# -slope
# -y-intercept
# - r, a value which when squared offers a "fit" value of the regression
...
# This line of code creates a list of values for y, the output of the function and maps them.
# List is the "listing of the values" while map generates an array which tabulates
# ( think of a matrix) which maps the values of (in this case) the linreg function output
# to the value of x, the input.

model = list(map(linreg, x))

py.scatter(x, y) # plot a scatter of the original data
py.plot(x, model)# plot the linear regression model
py.show() # show the plot with the combined data and model

print(r) # pearson correlation coefficient. The sign of the value
...
# the closer to 1, the better: suggesting a 1 to 1 correspondence between
#model and data.
...
# how well the model actually fits the observed data.